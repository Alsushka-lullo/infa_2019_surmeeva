...
for i in range(0, len(list_of_names), 4):
    dict_of_scores[list_of_names[i].rstrip()] = float(list_of_names[i + 1]) / float(list_of_names[i + 2]) - \
                                                float(list_of_names[i + 3])  # creation of the dicnionary
    # name-score in time
sorted_dict_of_scores = sorted(dict_of_scores.items(), key=operator.itemgetter(1)) #sorts this dictionry by value
